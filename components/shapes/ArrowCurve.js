...
const ArrowCurve = ({ size, shapeSettings, scale }) => {
  ...
  const controlY1 = startY + (endY - startY) * 0.5 + Math.abs(endX - startX) * 0.3;
  const controlX2 = startX + (endX - startX) * 0.75;
  const controlY2 = startY + (endY - startY) * 0.5 + Math.abs(endX - startX) * 0.3;
